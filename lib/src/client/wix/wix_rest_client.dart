...
class WixRestClient extends Client {
  WixRestClient({required super.authentication});

  static Future<WixRestClient> create() async {
    final authentication = await WixAuthentication.create();
    return WixRestClient(authentication: authentication);
  }
  ...
  Future<T> fetchItem<T extends Item>({
    ...
    final item = await FetchItemEndpoint<T>(
            accessToken: authentication.accessToken, itemType: itemType, id: id)
        .call();
